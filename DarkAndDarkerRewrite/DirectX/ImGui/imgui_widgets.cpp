...
void ImGui::LabelTextV(const char* label, const char* fmt, va_list args)
{
    ...
    // Render
    RenderTextClipped(ImVec2(value_bb.Max.x + 100 - value_size.x, value_bb.Min.y + style.FramePadding.y), value_bb.Max + value_bb.Max, value_text_begin, value_text_end, &value_size, ImVec2(0.0f, 0.0f));
    if (label_size.x > 0.0f)
        RenderText(ImVec2(value_bb.Min.x + style.FramePadding.x, value_bb.Min.y + style.FramePadding.y), label);
}
...
bool ImGui::Combo(const char* label, int* current_item, const std::vector<const char*> items, int height_in_items)
{
    const bool value_changed = Combo(label, current_item, Items_ArrayGetter, (void*)items.data(), items.size(), height_in_items);
    return value_changed;
}
